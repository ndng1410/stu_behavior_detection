...
After tuning
"""
X_train_val,X_test,y_train_val,y_test = train_test_split(merged_df.drop(['status'], axis=1),
                                                        merged_df['status'], 
                                                        test_size=0.2, random_state=42)
...
lr_et = time() - start_time
print(f'Logistic Regression Elapsed Time: {lr_et}')
...
rf_et = time() - start_time
print(f'Random Forest Elapsed Time: {rf_et}')

sns.heatmap(confusion_matrix(y_test, y_test_pred_rf), annot=True, fmt='d')
plt.title('Accuracy Score: {}'.format(accuracy_score(y_test, y_test_pred_rf)))
plt.ylabel('Actual')
plt.xlabel('Predicted')
plt.show()
...
vc_et = time() - start_time + lr_et + rf_et
print(f'Voting Classifier Elapsed Time: {vc_et}')

sns.heatmap(confusion_matrix(y_test, y_test_pred_voting), annot=True, fmt='d')
plt.title('Accuracy Score: {}'.format(accuracy_score(y_test, y_test_pred_voting)))
plt.ylabel('Actual')
plt.xlabel('Predicted')
plt.show()
